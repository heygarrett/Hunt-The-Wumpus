...
getDirection s p m = 
    if s == "back" 
        then
            ...
            then if prevLoc p == getAdjRooms p m !! 1 
                then return $ maximum (getAdjRooms p m)
                else if prevLoc p == head (getAdjRooms p m) 
                    then
            ...
            then if prevLoc p == getAdjRooms p m !! 1 
                then return $ minimum (getAdjRooms p m)
            else if prevLoc p == head (getAdjRooms p m) 
                then return $ maximum (getAdjRooms p m)
            else
                return $ getAdjRooms p m !! 1
...
checkForWumpus p m w =
    if wloc w `elem` getAdjRooms p m
        then return 0
        else if wloc w == location p
            then if moved w == 0
                then return 1
                else
                    return 2
